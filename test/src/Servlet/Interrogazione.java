...
public class Interrogazione extends HttpServlet {
	...
	public static ArrayList<String> ListaClientiNonClientiBusiness(){
		...
		try {
			...
			if(rs != null){
				while(rs.next()) { 
					array.add(rs.getString("email_utente"));
				}
			}
		} catch (SQLException e) {}
		finally {
	...
	public static ArrayList<Modello_Macchina> getModelliCS(){
		String sql ="SELECT * FROM (SELECT DISTINCT id_modello from macchina_cs) AS MCS INNER JOIN modello_macchina AS MOD ON MCS.id_modello = MOD.id";
		ResultSet rs = null;
		try {
			rs = Query.doQueryRS(sql);
			if(rs != null){
				ArrayList<Modello_Macchina> modelli = new ArrayList<Modello_Macchina>();
				while(rs.next()){
					modelli.add(new Modello_Macchina(rs.getInt("id"), rs.getString("marca"), rs.getString("modello"), rs.getInt("Cilindrata"), rs.getInt("anno"), rs.getString("alimentazione"), rs.getInt("emissioni_co2"), rs.getInt("num_posti")));
				}
				return modelli;
			}
		}catch (SQLException e){
